...
class CargosController extends Controller
{
    ...
    public function store(StoreCargoRequest $request)
    {
        ...
        try {
            ...
            $cargoTemAprovacao = (bool) ($configuracaoFluxo->cargo_tem_aprovacao ?? false);
            $fluxoAprovacaoId = $cargoTemAprovacao
                ? ($configuracaoFluxo->cargo_fluxo_aprovacao_id ?? null)
                : null;

            $statusAprovacao = $cargoTemAprovacao && $fluxoAprovacaoId
                ? 'pendente_aprovacao'
                : 'ativo';
            ...
            $aprovacaoSolicitacaoId = $this->iniciarFluxoAprovacaoCargoSeConfigurado($cargoId, $fluxoAprovacaoId);

            if ($aprovacaoSolicitacaoId) {
                DB::table('cargos')
                    ->where('id', $cargoId)
                    ->update([
                        'aprovacao_solicitacao_id' => $aprovacaoSolicitacaoId,
            ...
            return redirect()
                ...
                ->with('success', $fluxoAprovacaoId
                    ...
                    : 'Cargo salvo com sucesso e ativado.');
        } catch (Throwable $e) {
            DB::rollBack();
    ...
    {
        try {
            if (!DB::getSchemaBuilder()->hasTable('gestao_configuracao')) {
                return null;
            }

            return DB::table('gestao_configuracao')
                ->orderBy('id')
                ->first();
        } catch (Throwable $e) {
            return null;
    ...
    private function iniciarFluxoAprovacaoCargoSeConfigurado(int $cargoId, ?int $fluxoAprovacaoId): ?int
    {
        if (!$fluxoAprovacaoId) {
            return null;
        }
        ...
        /*
         * Ponto de entrada do motor de aprovação.
         * Nesta etapa, o gancho fica pronto, mas ainda sem criar a solicitação.
         */
        return null;
    }
